...
const presentationSlice = createSlice({
  ...
  reducers: {
    ...
    addImage(
      state,
      action: PayloadAction<{ slideId: ID; src: string; size?: Size; position?: Position }>,
    ) {
    ...
    reorderSlides(state, action: PayloadAction<{ indices: number[]; targetId: ID | null }>) {
      ...
      const unique = Array.from(
        new Set(indices.filter((i) => i >= 0 && i < state.slides.length)),
      ).sort((a, b) => a - b);
      ...
      const insertAt = targetId
        ? Math.max(
            0,
            rest.findIndex((s) => s.id === targetId),
          )
        : rest.length;
      state.slides = [...rest.slice(0, insertAt), ...picked, ...rest.slice(insertAt)];
      state.selection = {
        slideId: picked[0]?.id ?? state.selection.slideId ?? null,
        elementIds: [],
      };
    },
  },
